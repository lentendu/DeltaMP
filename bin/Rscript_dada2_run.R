...
list_seqtab<-data.frame(seqtab=list.files(pattern=".dada2_seqtab.rds$"),stringsAsFactors=F) %>%
  ...
  select(-suf)
if (nrow(list_seqtab) > 1) {
  seqtab<-dlply(list_seqtab,.(lib,comb),function(x) readRDS(x$seqtab) %>% .[,nchar(colnames(.))>=minlen]) %>% mergeSequenceTables(tables=.)
} else {
  seqtab<-readRDS(list_seqtab$seqtab) %>% .[,nchar(colnames(.))>=minlen]
}
